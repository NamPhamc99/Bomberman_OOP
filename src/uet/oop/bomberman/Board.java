...
public class Board implements IRender {
	...
	protected PlaySound _soundTrack;
	private Random _random;

	public Entity[] _entities;
	public List<Character> _characters = new ArrayList<>();
	protected List<Bomb> _bombs = new ArrayList<>();
	protected Queue<RemoteBomb> _remoteBombs = new LinkedList<RemoteBomb>();
	...
	public void loadLevel(int level) {
		...
		_remoteBombs.clear();
		_messages.clear();
		Game.resetBomberStatus();

		_soundTrack.stopSound();
        _soundTrack.playSound(".\\res\\SoundTrack\\GamePlay\\SoundTrack" + (_random.nextInt(3) + 1) + ".wav",true);
	...
	public Entity getEntity(double x, double y, Character m) {
		...
		res = getFlameSegmentofBombAt((int)x, (int)y);
		if( res != null) return res;

		res = getFlameSegmentofRemoteBombAt((int)x, (int)y);
		if( res != null) return res;

		res = getBombAt(x, y);
		if( res != null) return res;

		res = getRemoteBombAt(x, y);
		if( res != null) return res;
	...
	public Queue<RemoteBomb> getRemoteBombs() {
		return _remoteBombs;
	}
	...
	public RemoteBomb getRemoteBombAt(double x, double y){
		Iterator<RemoteBomb> bs = _remoteBombs.iterator();
		RemoteBomb rb;
		while(bs.hasNext()) {
			rb = bs.next();
			if(rb.getX() == (int)x && rb.getY() == (int)y)
				return rb;
		}
	...
	public FlameSegment getFlameSegmentofBombAt(int x, int y) {
		Iterator<Bomb> bs = _bombs.iterator();
		Bomb b;
		while(bs.hasNext()) {
			b = bs.next();
	...
	public FlameSegment getFlameSegmentofRemoteBombAt(int x, int y) {
		Iterator<RemoteBomb> bs = _remoteBombs.iterator();
		RemoteBomb rb;
		while(bs.hasNext()) {
			rb = bs.next();

			FlameSegment e = rb.flameAt(x, y);
			if(e != null) {
				return e;
			}
		}
	...
	public void addRemoteBombs(RemoteBomb rb){
		_remoteBombs.add(rb);
	}
	...
	protected void renderRemoteBombs(Screen screen){
		Iterator<RemoteBomb> itr = _remoteBombs.iterator();
	...
	protected void updateRemoteBombs() {
		if( _game.isPaused() ) return;
		Iterator<RemoteBomb> itr = _remoteBombs.iterator();
